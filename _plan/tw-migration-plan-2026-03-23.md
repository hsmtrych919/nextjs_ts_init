...
└── global/
    ...
    │   ├── _typ.scss              ← tw_source から（WP 専用除去）
...
| `features/_typ.scss` | `src/styles/global/features/_typ.scss` | WP 専用クラス除去（§7 参照） |

### tw_source から持ち込まないファイル（WP 専用）

- `features/_form.scss`, `_post.scss`, `_post-single.scss`, `_search.scss`, `_pagenation.scss`, `_google-map.scss`, `_login.scss`
...
| `component/react-modal/*` | `vendor/react-modal/*` | import パス修正 |
| `project/_style.scss` | `features/_style.scss` | `:root` CSS 変数定義を除去（_tailwind-base-layer.scss に移行）、grid.module import 除去、`@layer components` で囲む。`inview__fadein` / `is-inview` はそのまま維持（useInView.ts が IntersectionObserver + CSS transition で使用） |
| `modules/button.module.scss` | 据え置き | SCSS 色変数→CSS 変数変換、import パス修正 |
| `modules/header.module.scss` | 据え置き | import パス修正 |
| `modules/footer.module.scss` | 据え置き | import パス修正 |
| `modules/type.module.scss` | 据え置き | 内容更新（§7 参照）、import パス修正 |
| `modules/index.module.scss` | 据え置き | import パス修正 |
| `modules/modal-photo.module.scss` | 据え置き | import パス修正 |
| `modules/modal-footer.module.scss` | 据え置き | import パス修正 |
...
`g.rem()`, `g.$sm`, `g.$md` 等 — tw_source の global/ にすべて残っている。import パスの修正のみ。
...
- tw_source から `features/_typ.scss` を持ち込み（WP 専用除去。type.module.scss と重複するクラスは _typ.scss 側では除外するか、グローバル用と module 用で整理する — 要判断）
- react-modal 関連を `vendor/react-modal/` に移動
- `utility/_scroll-hint.scss` → `vendor/scroll-hint/_index.scss` に移動
- `_toggle.scss` の `@use "../mixins" as m;` 削除、`m.hover` → `g.hover` に変更
- 各 module.scss の import パス修正:
  - `@use "../global/global" as g;` → 維持（パス変更不要）
  - `@use "../global/mixins" as m;` → 廃止
- button.module.scss: SCSS 色変数→CSS 変数
- type.module.scss: §7 の方針で内容更新
...
### Step 6: grid.module.scss / gutter.module.scss 廃止 + TSX 書き換え
...
1. **features/_typ.scss と type.module.scss の責務分離**: tw_source の _typ.scss はグローバルクラス（`.typ`, `.ttl__*`）を `@layer components` で定義している。一方 Next.js の type.module.scss は同等のクラスを CSS Modules で定義している。両方に同じクラスが存在すると重複する。どちらか一方に寄せるか、グローバル用（テンプレートで直接使う）と module 用（TSX で import して使う）で分けるか、方針決定が必要。

2. **_style.scss の WP 専用コンテンツ**: tw_source の _style.scss には `dl-menu`, `dl-company`, `replace__*`, `swiper-front__*` 等がある。これらは WP 案件固有。Next.js の _style.scss は現行の内容（inview, swiper コメントアウト等）をベースに、tw_source からは `@layer components` の囲みだけを適用するのが妥当か。
